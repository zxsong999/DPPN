...
def parse_args():
    parser = argparse.ArgumentParser(description='DPPN training (Stage-1)')
...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    for epoch in range(config.num_epochs):
        ...
        acc1, ece, true_class, pred_class, confidence = validate(val_loader, model, classifier, criterion, config, logger, block)
        ...
        if is_best:
            its_ece = ece
            reliability_diagram(config,true_class, pred_class, confidence, 10)
...
def validate(val_loader, model, classifier, criterion, config, logger, block=None):
    ...
    return top1.avg, cal['expected_calibration_error'] * 100, true_class, pred_class, confidence
