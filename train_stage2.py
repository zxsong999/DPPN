...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    # criterion = BatchDynamicsLoss(config).cuda(config.gpu)
    ...
    optimizer = torch.optim.SGD([{"params": classifier.parameters()}], config.lr,
    ...
    for epoch in range(config.num_epochs):
        ...
        if config.dataset != 'places':
            block = None
        # train for one epoch
        train(train_loader, model, classifier, criterion,is_best, optimizer, epoch, config, logger, block)

        # evaluate on validation set
        acc1, ece, true_class, pred_class, confidence, prob_class = validate(val_loader, model, classifier, criterion, config, logger, block)
        
        class_misclassification_values = probabilistic_bias(true_class, pred_class, prob_class, config.num_classes)
        logger.info("Misclassification Values for Each Class:")
        logger.info(class_misclassification_values)
        # for class_id, value in enumerate(class_misclassification_values):
        #     logger.info(f"Class {class_id}: {value}")
        ...
        if is_best:
            its_ece = ece
            reliability_diagram(config,true_class, pred_class, confidence, config.num_classes)
            W = classifier.module.fc.weight.cpu()
            tmp = torch.linalg.norm(W, ord=2, dim=1).detach().numpy()
            # 打开一个文本文件用于写入数据，如果文件不存在则创建它
            with open("./figure/"+str(config.name)+"_norm.txt", 'w') as file:
...
def train(train_loader, model, classifier, criterion, is_best,optimizer, epoch, config, logger, block=None):
        ...
        if config.mixup is True:
            ...
            with torch.no_grad():
                if config.dataset == 'places':
                    ...
                    feat = model(images)
            output = classifier(feat.detach())
            loss = criterion(output, target,is_best)
...
def validate(val_loader, model, classifier, criterion, config, logger, block=None):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if prob_class.size == 0:
                prob_class = prob.cpu().numpy()
            else:
                prob_class = np.vstack((prob_class,prob.cpu().numpy()))
    ...
    return top1.avg, cal['expected_calibration_error'] * 100, true_class, pred_class, confidence, prob_class
